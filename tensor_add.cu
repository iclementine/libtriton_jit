...
__global__ void tensor_add(
    ...
    int64_t* strides_a, int64_t* strides_b, int64_t* strides_out,
    int64_t* task_shape,
    int num_tasks, int rank
) {
...
at::Tensor add_tensor(const at::Tensor& a_, const at::Tensor& b_) {
    ...
    at::Tensor strides_a = at::empty({rank}, at::TensorOptions().dtype(torch::kInt64).device(a.device()));
    cudaMemcpy(strides_a.mutable_data_ptr<int64_t>(), a.strides().data(), rank * sizeof(int64_t), cudaMemcpyHostToDevice);

    at::Tensor strides_b = at::empty({rank}, at::TensorOptions().dtype(torch::kInt64).device(a.device()));
    cudaMemcpy(strides_b.mutable_data_ptr<int64_t>(), b.strides().data(), rank * sizeof(int64_t), cudaMemcpyHostToDevice);

    at::Tensor strides_out = at::empty({rank}, at::TensorOptions().dtype(torch::kInt64).device(a.device()));
    cudaMemcpy(strides_out.mutable_data_ptr<int64_t>(), out.strides().data(), rank * sizeof(int64_t), cudaMemcpyHostToDevice);

    at::Tensor task_shape = at::empty({rank}, at::TensorOptions().dtype(torch::kInt64).device(a.device()));
    cudaMemcpy(task_shape.mutable_data_ptr<int64_t>(), out.sizes().data(), rank * sizeof(int64_t), cudaMemcpyHostToDevice);
    ...
    tensor_add<T, tile_size, block_size><<<num_blocks, block_size>>>(
      ...
      strides_a.data_ptr<int64_t>(), strides_b.data_ptr<int64_t>(), strides_out.data_ptr<int64_t>(),
      task_shape.data_ptr<int64_t>(), out.numel(), rank);
    return out;
}
