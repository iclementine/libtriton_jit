...
# signature for jit function -> c++ function signature.
...
def generate_a_shim(
    ...
    # example "tensor, tensor, int64_t, int64_t"
    # for bool use i1, for boolean values, use 0 or 1.
    # split it
    signature: List[str] = list(map(lambda s: s.strip(" "), signature.split(",")))
    num_args = len(fn.signature.parameters)
    assert len(signature) == num_args

    # stream, grid, num-warps, num-stages, args
