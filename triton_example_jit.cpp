...
private:
  ...
  // TODO: find a faster way to represent key
...
public:
  ...
  void invoke(unsigned int grid_x, unsigned int grid_y, unsigned int grid_z,
    ...
    std::string signature = fmt::format(
        ...
        to_triton_typename(b.scalar_type()),
    ...
    if (pos == this->overloads_.end()) {
      std::string cmd = fmt::format(
          ...
          "{}",
          this->function_name_, signature, num_warps, num_stages, this->path_);
...
at::Tensor add_tensor(const at::Tensor &a_, const at::Tensor &b_) {
  ...
  f.invoke(num_blocks, 1, 1, num_warps, num_stages, a, b, out, n, tile_size,
           raw_stream);
  return out;
}
